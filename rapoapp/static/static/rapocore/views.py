...
def Browse(request):
    ...
    data = Book.objects.all().values('id','title','author__first_name','author__last_name','language__languagename','ownermember__user__first_name','ownermember__user__last_name','withmember__user__first_name','withmember__user__last_name','status','rqueue__id','rqueue__user__first_name','rqueue__user__last_name')
...
def Add2Queue(request):
    if request.method == "POST":
        form = Add2QueueForm(request.POST)
        if form.is_valid():
            try:
                newObject = form.save()
            except forms.ValidationError, error:
                newObject = None
            if newObject:
                return HttpResponse('<script type="text/javascript">opener.dismissAdd2Queue(window, "%s", "%s");</script>' % \
                    (escape(newObject._get_pk_val()), escape(newObject)))
    else:
        form = Add2QueueForm()

    return render_to_response('rapocore/add2queue.html',{ 'form': form , 'field' : 'book'}, RequestContext(request))
